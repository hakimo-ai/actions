...
| 13. Post PR comment | On `pull_request`/`pull_request_target` only (no-op otherwise). Reads the same three JSON files back off disk, builds a compact per-scanner count table plus the top `max-comment-findings` findings (most severe first) in a collapsed `<details>` block, and upserts a single comment (matched via an HTML marker, paginating through all existing comments so it never misses its own prior comment on an active PR). The marker includes the scan target (`image` if set, else `path`), so calling this action more than once on the same PR/push (e.g. a path-mode scan plus a separate image-mode `security-scan-image` job) posts one distinct comment per call instead of racing to overwrite a single shared one - same fix as `lint-check`'s path-suffixed marker. |
